...
namespace AoC2021DotNet.AoC
{
    public class Day8
    {
        ...
        public void Part2()
        {
            ...
            foreach (var encoded in data)
            {
                var value = Decode(encoded);
                Console.WriteLine($"{encoded.Last()}: {value}");

                values.Add(value);
            }

            Console.WriteLine($"Sum of all output values: {values.Sum()}");
        }

        private long Decode(IEnumerable<string> line)
        {
            // Find mapping
            var mapping = new Dictionary<int, string>();
            var signalPatterns = line.First()
                .Split(" ")
                .Select(Order)
                .OrderBy(x => x)
                .ToList();
            ...
            // Numbers with a distinct number of segments (1, 4, 7, 8)
            var distinctNumberOfSignals = new Dictionary<int, int>() { { 1, 2 }, { 4, 4 }, { 7, 3 }, { 8, 7 } };
            foreach (var (number, numberOfSegments) in distinctNumberOfSignals)
            {
                var pattern = signalPatterns.FirstOrDefault(x => x.Length == numberOfSegments);
                if (pattern != null)
                {
                    mapping.Add(number, Order(pattern));
                }
            }

            // Three: Length five and full overlap with one
            var threePattern =
                signalPatterns.Find(p => p.Length == 5 && mapping[1].All(p.Contains));
            mapping.Add(3, threePattern);

            // Six: Has six segments and has no full overlap with one
            var sixPattern =
                signalPatterns.Find(p => p.Length == 6 && !mapping[1].All(p.Contains));
            mapping.Add(6, sixPattern);

            // Nine
            var ninePattern = signalPatterns.Find(p =>
            {
                var threeFourCombi = mapping[3] + mapping[4];
                return p.Length == 6 && threeFourCombi.All(p.Contains);
            });
            mapping.Add(9, ninePattern);

            // Zero: has length six and is not pattern for nine or six
            var zeroPattern = signalPatterns.Find(p => p.Length == 6 && p != ninePattern && p != sixPattern);
            mapping.Add(0, zeroPattern);

            // Five: Length five and is full overlapped by nine and not by one
            var fivePattern = signalPatterns.Find(p => p.Length == 5 && p.All(mapping[9].Contains) && !mapping[1].All(p.Contains));
            mapping.Add(5, fivePattern);

            // Two: Length five and is not five or three
            var twoPattern = signalPatterns.Find(p => p.Length == 5 && p != fivePattern && p != threePattern);
            mapping.Add(2, twoPattern);

            var flippedMapping = mapping.ToDictionary(kvp => kvp.Value, kvp => kvp.Key);

            // Decode
            var values = line.Last()
                .Split(" ")
                .Select(Order)
                .Select(pattern => flippedMapping[pattern].ToString());

            return long.Parse(string.Join("", values.ToArray()));
        }

        private string Order(string input)
        {
